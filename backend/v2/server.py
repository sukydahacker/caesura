...
from services.printrove_service import printrove_service
...
def calculate_print_size(product_type: str, analysis: dict) -> dict:
    ...
    presets = {
        ...
        "bomber_jacket": {"max_width_cm": 30, "max_height_cm": 35, "scale_ratio": 0.8},
        "sweatshirt": {"max_width_cm": 36, "max_height_cm": 44, "scale_ratio": 0.85}
    }
...
async def create_order(request: Request, session_token: Optional[str] = Cookie(None)):
    ...
    # Process each item for Printrove fulfillment and revenue split
    printrove_order_products = []
    for item in body["items"]:
        ...
        if product:
            ...
            if product.get("printrove_product_id"):
                order_product["product_id"] = product["printrove_product_id"]
            if product.get("printrove_design_id"):
                design_dims = product.get("printrove_design_dimensions", {})
                order_product["design"] = {
                    "front": {
                        "id": product["printrove_design_id"],
                        "dimensions": design_dims.get("front", {"width": 38, "height": 48, "top": 7, "left": 0})
                    }
                }
            printrove_order_products.append(order_product)

    if printrove_order_products:
        try:
            printrove_order = await printrove_service.create_order(
                reference_number=order_id,
                retail_price=body["total_amount"],
                customer=body["shipping_address"],
                order_products=printrove_order_products,
                cod=False,
            )
            await db.orders.update_one(
                {"order_id": order_id},
                {"$set": {
                    "printrove_order_id": printrove_order.get("id"),
                    "fulfillment_provider": "printrove",
                    "tracking_number": printrove_order.get("tracking_number"),
                    "tracking_url": printrove_order.get("tracking_url")
                }}
            )
            logger.info(f"Printrove order placed for {order_id}: {printrove_order.get('id')}")
...
async def approve_design_admin(design_id: str, request: Request, session_token: Optional[str] = Cookie(None)):
    ...
    # Upload design to Printrove and create product
    printrove_design_id = None
    printrove_product_id = None
    try:
        # Upload design to Printrove design library
        printrove_design = await printrove_service.upload_design(
            image_url=design["image_url"],
            name=design["title"]
        )
        printrove_design_id = printrove_design.get("id")

        # Determine product type and build variants from product_configs
        product_configs = design.get("product_configs", [])
        first_config = product_configs[0] if product_configs else {}
        product_type = first_config.get("productType", "tshirt")

        # Map our product types to Printrove parent product IDs (real catalog IDs)
        printrove_product_map = {
            "tshirt": 460,            # Half Sleeve Round Neck T-Shirt
            "hoodie": 463,            # Hoodies
            "oversized_tshirt": 1216, # Oversized Tshirts
            "bomber_jacket": 1387,    # Bomber Jacket
            "sweatshirt": 1012,       # Sweatshirts
        }
        parent_product_id = printrove_product_map.get(product_type, 460)

        # Calculate design dimensions from print metadata
        print_size = calculate_print_size(product_type, design.get("design_analysis"))
        design_dimensions = {
            "width": int(print_size["width_cm"] * 10),  # Convert cm to mm for Printrove
            "height": int(print_size["height_cm"] * 10),
            "top": 70,   # Default 7cm from top
            "left": 0,   # Centered
        }

        # Create variants (all sizes for the selected color)
        variants = [{"product_id": parent_product_id}]

        # Create product on Printrove
        if printrove_design_id:
            printrove_product = await printrove_service.create_product(
                name=design["title"],
                parent_product_id=parent_product_id,
                design_id=printrove_design_id,
                variants=variants,
                design_dimensions=design_dimensions,
            )
            printrove_product_id = printrove_product.get("id")

    except Exception as e:
        logger.error(f"Failed to create Printrove product: {e}")

    # Determine apparel type from product configs
    product_configs = design.get("product_configs", [])
    first_config = product_configs[0] if product_configs else {}
    apparel_type = first_config.get("productType", "tshirt")
    base_price = first_config.get("basePrice", 999)
    ...
    product_doc = {
        ...
        "printrove_product_id": printrove_product_id,
        "printrove_design_id": printrove_design_id,
        "printrove_design_dimensions": {"front": {"width": 380, "height": 480, "top": 70, "left": 0}},
...
@api_router.get("/admin/printrove/categories")
async def get_printrove_categories(request: Request, session_token: Optional[str] = Cookie(None)):
    await require_admin(request, session_token)
    categories = await printrove_service.get_categories()
    return {"categories": categories, "mock_mode": printrove_service.mock_mode}

@api_router.get("/admin/printrove/categories/{category_id}")
async def get_printrove_category_products(category_id: str, request: Request, session_token: Optional[str] = Cookie(None)):
    await require_admin(request, session_token)
    products = await printrove_service.get_category_products(category_id)
    return {"products": products}

@api_router.get("/admin/printrove/designs")
async def get_printrove_designs(request: Request, session_token: Optional[str] = Cookie(None)):
    await require_admin(request, session_token)
    designs = await printrove_service.list_designs()
    return designs

@api_router.get("/admin/printrove/orders")
async def get_printrove_orders(request: Request, session_token: Optional[str] = Cookie(None)):
    await require_admin(request, session_token)
    orders = await printrove_service.get_orders()
    return orders

@api_router.get("/admin/printrove/orders/{order_id}")
async def get_printrove_order(order_id: str, request: Request, session_token: Optional[str] = Cookie(None)):
    await require_admin(request, session_token)
    order = await printrove_service.get_order(order_id)
    return order

@api_router.get("/api/serviceability")
async def check_serviceability(country: str = "IN", pincode: str = "", weight: int = 200, cod: bool = False):
    result = await printrove_service.check_serviceability(country, pincode, weight, cod)
    return result
